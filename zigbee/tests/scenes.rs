...
use common::{
    at, command, device, events, group_command, join_group, joined_device, recall_scene,
    store_scene, transmissions,
};
use zigbee::{Colour, Event, CLUSTER_GROUPS, CLUSTER_SCENES};
...
fn storing_a_scene_and_recalling_it_puts_the_light_back() {
    ...
    store_scene(&mut device, LOOSE, 1, 10);

    device.set_level(20);
    device.set_on_off(false);

    recall_scene(&mut device, LOOSE, 1, 20);
...
fn a_recall_puts_the_colour_back_too() {
    ...
    store_scene(&mut device, LOOSE, 2, 10);
    ...
    recall_scene(&mut device, LOOSE, 2, 30);
...
fn recalling_a_scene_nobody_stored_changes_nothing() {
    ...
    recall_scene(&mut device, LOOSE, 9, 10);
...
fn a_removed_scene_is_gone() {
    ...
    device.set_level(30);
    recall_scene(&mut device, LOOSE, 1, 30);
...
fn removing_every_scene_of_a_group_takes_them_all() {
    ...
    device.set_level(120);
    store_scene(&mut device, LOOSE, 1, 10);
    store_scene(&mut device, LOOSE, 2, 11);

    let arguments = LOOSE.to_le_bytes();
    device.receive(
        &command(CLUSTER_SCENES, 0x97, REMOVE_ALL, &arguments),
        at(20),
    );

    device.set_level(30);
    recall_scene(&mut device, LOOSE, 1, 30);
    recall_scene(&mut device, LOOSE, 2, 31);
...
fn a_scene_in_a_group_needs_the_light_to_be_in_that_group() {
    ...
    store_scene(&mut device, 7, 1, 10);
    device.set_level(30);
    recall_scene(&mut device, 7, 1, 20);
    assert_eq!(device.level(), 30, "the light was never in group 7");

    join_group(&mut device, 7, 0);
    device.set_level(120);
    store_scene(&mut device, 7, 1, 30);
    device.set_level(30);
    recall_scene(&mut device, 7, 1, 40);
    assert_eq!(device.level(), 120);
}
...
fn leaving_a_group_takes_its_scenes_with_it() {
    ...
    device.set_level(30);
    recall_scene(&mut device, 7, 1, 30);
...
fn a_scene_recalled_at_a_group_reaches_every_member() {
    ...
    store_scene(&mut device, 7, 3, 10);
    device.set_on_off(false);
    device.set_level(10);
    transmissions(&mut device);

    device.receive(
        &group_command(7, CLUSTER_SCENES, 0x99, RECALL, &[7, 0, 3]),
        at(20),
    );
...
fn the_table_holds_eight_scenes_and_refuses_the_ninth() {
    ...
    device.set_level(120);
    for scene in 1..=9u8 {
        store_scene(&mut device, LOOSE, scene, 10 + scene as u32);
    }

    device.set_level(30);
    recall_scene(&mut device, LOOSE, 9, 40);
    assert_eq!(device.level(), 30, "the ninth scene never fitted");

    recall_scene(&mut device, LOOSE, 8, 50);
