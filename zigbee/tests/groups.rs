...
use common::{at, command, device, group_command, joined_device, transmissions};
...
fn join_group(device: &mut Device, group: u16) {
    device.receive(
        &command(CLUSTER_GROUPS, 0x80, ADD, &[group.to_le_bytes()[0], group.to_le_bytes()[1], 0]),
        at(0),
    );
}
...
fn adding_a_group_only_while_identifying_waits_for_the_identify() {
    ...
    device.receive(&command(zigbee::CLUSTER_IDENTIFY, 0x86, 0x00, &[30, 0]), at(20));
    device.receive(
